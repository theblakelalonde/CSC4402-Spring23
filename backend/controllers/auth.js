...
export const register = (req, res) => {
    ...
    db.query(q, [req.body.username], (err, data) => {
        ...
        if (data.length) return res.status(409).json('User with this information already exists!')
...
export const login = (req, res) => {
    ...
    db.query(q, [req.body.username], (err, data) => {
        if (err) return res.status(500).json(err)
        if (data.length === 0) return res.status(404).json('User with this information does not exist!')
